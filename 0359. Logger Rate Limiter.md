...
In the method `couldPrintMessage()`, given a `message` and a `timestamp` in seconds, return _true_ if the message can be printed within the given timestamp, otherwise return _false_
...
class Logger:
    ...
    def could_print_message(self, timestamp: int, message: str) -> bool:
        ...
        if message not in self.dictmap:
            self.dictmap[message] = timestamp
            return True
        else:
            if timestamp - self.dictmap.get(message) >= 10:
                self.dictmap[message] = timestamp
                return True
            else:
                return False
